...
/**
 * The torque control function will be called in 1/40k sec
 *
 * @param controller - p_goal : Command from CAN bus
 *                                   - theta_mech : Current position of motor
 *                                   - flag : Be zero when receiving CAN command
 *                                   - p_des : The position goal
 *                                   - error_sum : The integral of error
 *                                   - kp,ki,kd : The parameters of controller
 *                                   - t_ff : Feedforward term
 */
float temp = 0.01;
float ki = 0;
void torque_control(ControllerStruct *controller)
{
    ...
    {
        controller->theta_mech -= int(controller->theta_mech / (6.28 * 6));
    ...
    float torque_ref = controller->kp * error + ki * controller->error_sum + controller->kd * (-controller->dtheta_mech) + controller->t_ff;
